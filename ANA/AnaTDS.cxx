...
void AnaTDS::analizeTDS(const std::string &fileRaw, const std::string &fileRoot, int &index){
  ...
  std::vector<Hit> myHits(TDS_MAX_SIGNALS);
      ...
      for (int nev=0; nev<nEvents; nev++){
        ...
        std::vector <Char_t>buffer(pulse_depth);
          ...
          for(int i=0; i<nHits;i++){
            ...
            Hit hit(i,index,pulse_depth,blockhead.mVdiv[i], srate, blockhead.Pretrigger,blockhead.NegPolarity[i], buffer);

            hit.SetClockTickLT (eventhead.clockTicksLT);
            hit.SetClockTickRT (eventhead.clockTicksRT);
            // Set delta time (real time since previous event)
              if(index==0)hit.SetDeltaTime(0);
              else hit.SetDeltaTime ( eventhead.clockTicksRT - prevTime);
